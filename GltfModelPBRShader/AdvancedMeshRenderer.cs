...
namespace Game {
    ...
    public abstract class AdvancedMeshRenderer : ICustomModelRenderer {
        ...
        public virtual void BeginFrame(Camera camera) {
            ...
            Engine.Vector3 moonDir = Engine.Vector3.Normalize(
                Engine.Vector3.Transform(
                    Engine.Vector3.UnitY,
                    Engine.Matrix.CreateRotationZ(-moonAngle) * Engine.Matrix.CreateRotationX(seasonAngle)
                )
            );

            // Sun color with dawn/dusk glow
            float dawnGlow = Math.Max(
                _subsystemSky.CalculateDawnGlowIntensity(timeOfDay),
